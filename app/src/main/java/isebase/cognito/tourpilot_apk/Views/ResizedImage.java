	...
	public void draw(Canvas canvas, int zoomIndex, Paint paint) {
		...
		for (KontrakturPoint activePoint : activePoints) {
			paint.setColor(StaticResources
					...
					.getResources()
					.getColor(
							activePoint.state == 0 ? isebase.cognito.tourpilot_apk.R.color.green
									: activePoint.state == 1 ? isebase.cognito.tourpilot_apk.R.color.blue
											: isebase.cognito.tourpilot_apk.R.color.red));
